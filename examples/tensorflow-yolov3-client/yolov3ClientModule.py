...
class Yolov3RemoteInference(object):
    def __init__(self, protocol="grpc", url="localhost:8001", 
        model_name="yolov3", model_version=1, batch_size=1, verbose=False):
...
# @logger.catch
def AlgorithmInitModle(GPUID, logPath):

    logger.add(logPath, rotation="1 MB", backtrace=False, level="DEBUG")
    logger.info("python: GPUID received: %d" % GPUID)
    logger.info("logPath received: %s\n" % logPath)
...
def AlgorithmRun(cameraID, timeStamps, img):
    ...
    return ret, result, detectResult, image