...
{
	char buf[4096];
	read_profile_string(m_section.c_str(),key.c_str(),default_value.c_str(),buf,sizeof(buf),m_fileName.c_str());
	return buf;
}
...
// 									const char * t = p+newline_start;
									int len = j-newline_start;
									if(key_len == len && strncmp(key,p+newline_start,len)==0)
									{
										//find the key ok
										*key_s = newline_start;
										*key_e = j-1;
...
*/
int IniFile::read_profile_string( const char *section, const char *key,const char *default_value,char *value, 
...
{
	char value[32] = {0};
	if(!read_profile_string(section,key,NULL,value, sizeof(value),file))
	{
...
string IniFile::readStrEnv(const string &key, const string &default_value) const
{
	char* value = NULL;
...
int IniFile::readIntEnv(const string &key, int default_value) const
{
	char* value = NULL;
	value = getenv(key.c_str());
	if(value == NULL)
