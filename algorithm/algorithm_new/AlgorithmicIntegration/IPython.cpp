...
namespace ipython {
	...
	// 初始化python运行环境成功返回true
	// 设置python环境变量，初始化线程支持，设置python搜索路径
	bool InitPython()
	{
#if WIN32
		...
		PyRun_SimpleString("sys.path.insert(0,'mount/')");
...
//#ifndef _Debug_
		...
		PyTuple_SetItem(pArg, 3, Py_BuildValue("s", dynamicArgs.c_str()));//add
