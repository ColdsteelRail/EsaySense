...
void pic_mgr::run()
{
	while (thread_.IsWorking())
	{
		...
		bool ret = NetLib_SnapshotAndUpload(frame->rgb_buf, frame->size, frame->width, frame->height, strfile.c_str(),
			SettingConfig::GetInstance()->m_ftp_host, SettingConfig::GetInstance()->m_ftp_user, SettingConfig::GetInstance()->m_ftp_pwd);
